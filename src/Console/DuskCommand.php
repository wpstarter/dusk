...
class DuskCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $options = ws_collect($_SERVER['argv'])
            ->slice(2)
    ...
    protected function phpunitArguments($options)
    {
        ...
        if (! file_exists($file = ws_base_path('phpunit.dusk.xml'))) {
            $file = ws_base_path('phpunit.dusk.xml.dist');
        }
    ...
    protected function purgeScreenshots()
    {
        $this->purgeDebuggingFiles(
            ws_base_path('tests/Browser/screenshots'), 'failure-*'
        );
    }
    ...
    {
        $this->purgeDebuggingFiles(
            ws_base_path('tests/Browser/console'), '*.log'
        );
    }
    ...
    {
        $this->purgeDebuggingFiles(
            ws_base_path('tests/Browser/source'), '*.txt'
        );
    }
    ...
    {
        if (file_exists(ws_base_path($this->duskFile()))) {
            if (file_get_contents(ws_base_path('.env')) !== file_get_contents(ws_base_path($this->duskFile()))) {
    ...
    {
        copy(ws_base_path('.env'), ws_base_path('.env.backup'));

        copy(ws_base_path($this->duskFile()), ws_base_path('.env'));
    }
    ...
    protected function refreshEnvironment()
    {
        ...
        if (! method_exists(Dotenv::class, 'create')) {
            (new Dotenv(ws_base_path()))->overload();
        ...
        if (! method_exists(Dotenv::class, 'createMutable')) {
            Dotenv::create(ws_base_path())->overload();

            return;
        }

        Dotenv::createMutable(ws_base_path())->load();
    }
    ...
    {
        if (! file_exists($file = ws_base_path('phpunit.dusk.xml')) &&
            ! file_exists(ws_base_path('phpunit.dusk.xml.dist'))) {
    ...
    protected function teardownDuskEnviroment()
    {
        ...
        if (file_exists(ws_base_path($this->duskFile())) && file_exists(ws_base_path('.env.backup'))) {
    ...
    {
        if (! $this->hasPhpUnitConfiguration && file_exists($file = ws_base_path('phpunit.dusk.xml'))) {
            unlink($file);
        }
    }
    ...
    {
        copy(ws_base_path('.env.backup'), ws_base_path('.env'));

        unlink(ws_base_path('.env.backup'));
    }
    ...
    {
        if (file_exists(ws_base_path($file = '.env.dusk.'.$this->laravel->environment()))) {
            return $file;
        }
